from util import utc_str_to_local, print_input_menu, valid_response_digitalocean, print_output_table, print_text_prompt, yellow_text
...
class Firewall:
    ...
    def get_firewalls(self):
        """
        Retrieves the list of firewall groups from the Digital Ocean API, presents them to the user for selection,
        ...
        print(data)
        if valid_response_digitalocean(data):
            option, fw_list = print_input_menu(data['firewalls'], 'What firewall to select?: ', 'id', ['name'], True)
    ...
    #     """
    #     Retrieves the details of the currently selected firewall group from the Digital Ocean API.

    #     If a firewall group is selected, this method sends a GET request to the Digital Ocean API to fetch
    ...
    #         data = self.api.api_get(url)
    #         if valid_response_Digital Ocean(data):
    #             self.firewall_desc = data['firewall_group']['description']

    def print_firewall(self):
        """
        Prints detailed information about the currently selected firewall.

        This method retrieves the firewall details from the Digital Ocean API using the selected firewall ID.
        If the API response is valid, it displays the firewall's description, creation and modification dates
        ...
        if self.firewall_selected():
            ...
            if valid_response_digitalocean(data):
                result = [
                    ['Name: ', data['firewalls']['name']],
                    ['Status: ', data['firewalls']['status']],
                    ['Date Created: ', utc_str_to_local(data['firewalls']['created_at'])],
                    ['Droplets: ', data['firewalls']['droplet_ids']],
                ]
    ...
    #     """
    #     Deletes the firewall associated with this instance using the Digital Ocean API.

    #     Sends a DELETE request to the Digital Ocean API for the firewall identified by `self.firewall_id`.
    ...
    #     If a firewall is selected, this method sends a GET request to the Digital Ocean API to fetch the rules associated with the selected firewall.
    ...
    #         Also assumes the existence of `self.api.api_delete()` and `valid_response_Digital Ocean()`.
    #     """
    #     if not self.firewall_selected():
    #         return
    #     self.get_firewall_rules()
    ...
    #     prompts the user to enter notes for the firewall rule (defaulting to 'Added by pyDigital Ocean' if left blank),
    ...
    #     if not self.firewall_selected():
    #         return
    #     notes = print_text_prompt("Notes for the firewall rules?['Added by pyDigital Ocean']: ")
    #     if notes == '':
    #         notes = 'Added by pyDigital Ocean'
